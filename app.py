...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #1a1f3a 0%, #0d1117 100%) !important;
...
[data-testid="stSidebar"] .stRadio > div > label {
    background: rgba(30, 41, 82, 0.4) !important;
    border: 1px solid rgba(112, 145, 230, 0.2) !important;
...
[data-testid="stSidebar"] .stRadio > div > label:has(input:checked) {
    background: rgba(112, 145, 230, 0.25) !important;
    color: #7091E6 !important;
    border-color: #7091E6 !important;
    border-left: 4px solid #7091E6 !important;
    font-weight: 600 !important;
    box-shadow: inset 0 0 12px rgba(112, 145, 230, 0.1) !important;
}
...
.stSlider > div > div > div { 
    background: linear-gradient(90deg, #7091E6 0%, #3fb950 100%) !important; 
}
...
hr { 
    border-color: #1e2947 !important; 
    margin: 16px 0 !important;
}

/* ── KPI CARDS ── */
.stat-card {
    background: linear-gradient(135deg, rgba(26, 32, 58, 0.8), rgba(30, 41, 82, 0.6)) !important;
    border: 1px solid #30363d !important;
...
.stat-card:hover { 
    ...
    box-shadow: 0 12px 48px rgba(112, 145, 230, 0.2) !important;
    background: linear-gradient(135deg, rgba(112, 145, 230, 0.1), rgba(30, 41, 82, 0.8)) !important;
}
...
.section-hdr {
    ...
    border-bottom: 2px solid #1e2947 !important;
}
...
.sec-badge {
    ...
    background: rgba(112, 145, 230, 0.2) !important; 
    color: #7091E6 !important;
    border: 1px solid rgba(112, 145, 230, 0.4) !important; 
    text-transform: uppercase !important; 
...
.ss-row {
    ...
    border-bottom: 1px solid #1e2947 !important;
}
...
[data-testid="stDataFrame"] {
    background: rgba(26, 32, 58, 0.5) !important;
    border: 1px solid #30363d !important;
}
</style>
...
def mfig(w=10, h=4, ncols=1, nrows=1):
    ...
    for ax in alist:
        ...
        ax.tick_params(colors=TK, labelsize=9, length=0, pad=6)
        ...
        for s in ax.spines.values():
            s.set_edgecolor(GRD)
            s.set_linewidth(1)
        ax.grid(axis='y', color=GRD, linewidth=0.6, linestyle='--', alpha=0.4)
...
with st.sidebar:
    st.markdown("""
    <div style='padding:28px 18px 24px; border-bottom: 2px solid #30363d; margin-bottom: 20px;'>
        <div style='display: flex; align-items: center; gap: 10px; margin-bottom: 8px;'>
            <div style='width: 36px; height: 36px; background: linear-gradient(135deg, #7091E6, #4a6cf7); border-radius: 10px; display: flex; align-items: center; justify-content: center;'>
                <span style='font-size: 20px;'>📊</span>
            </div>
            <div>
                <div style='font-size: 20px; font-weight: 800; color: #f8f9fa; letter-spacing: -0.5px;'>
                    Sales<span style='color: #7091E6;'>IQ</span>
                </div>
                <div style='font-size: 11px; color: #7691c0; font-weight: 600;'>
                    Analytics Platform
                </div>
            </div>
        </div>
    ...
    st.markdown("<div style='font-size: 11px; color: #7691c0; text-transform: uppercase; letter-spacing: 1.2px; padding: 0 6px 12px; font-weight: 800;'>NAVIGATION</div>", unsafe_allow_html=True)
    ...
    st.markdown("<hr style='margin: 20px 0; border-color: #1e2947;'/>", unsafe_allow_html=True)
    
    st.markdown(f"""
    <div class='sidebar-stats'>
        <div style='font-size: 11px; color: #7691c0; text-transform: uppercase; letter-spacing: 1.2px; font-weight: 800; padding-bottom: 14px;'>QUICK STATS</div>
...
if page == "📊 Sales Overview":
    ...
    st.markdown("<p>Full-year performance across all channels and regions</p>", unsafe_allow_html=True)
    ...
    ax.plot(x, y, color=C2, lw=2.8, zorder=3, label='Revenue')

    ma = monthly['Sales'].rolling(3).mean() / 1e3
    ax.plot(x, ma, color=C3, lw=1.6, ls='--', alpha=0.7, label='3-month Moving Avg', zorder=2)

    pidx = monthly['Sales'].idxmax()
    ax.scatter(x.iloc[pidx], y.iloc[pidx], color=UP, s=120, zorder=6, edgecolors=BG, lw=2.5)
    ax.annotate(f"Peak: ${y.iloc[pidx]:.0f}K",
                (x.iloc[pidx], y.iloc[pidx]),
                xytext=(10, 14), textcoords='offset points',
                color=UP, fontsize=9, fontweight='800',
                bbox=dict(boxstyle='round,pad=0.6', facecolor=BG, edgecolor=UP, alpha=0.8, linewidth=1.5))

    ax.xaxis.set_major_formatter(mdates.DateFormatter('%b %y'))
    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=3))
    plt.setp(ax.get_xticklabels(), rotation=30, ha='right', color=TK)
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f'${x:.0f}K'))
    legend = ax.legend(facecolor=PNL, edgecolor=GRD, labelcolor=C5, fontsize=9, loc='upper left', framealpha=0.95)
    finish(fig)
    st.pyplot(fig)
    ...
    with col1:
        st.markdown("""<div class='section-hdr'>
            <span class='sec-icon'>📅</span><span class='sec-title'>By Year</span>
        </div>""", unsafe_allow_html=True)
        ...
        for bar, val in zip(bars, yearly.values):
            ax2.text(bar.get_x() + bar.get_width() / 2, val / 1e3 + 1,
                     ...
                     color=C5, fontsize=10, fontweight='800')
    ...
    with col2:
        ...
        cat_sel = st.multiselect("Select categories", df['Category'].unique().tolist(),
                                 ...
                                 key="cat_select", label_visibility='collapsed')
        cat = df[df['Category'].isin(cat_sel)].groupby('Category')['Sales'].sum().sort_values()
        fig3, ax3 = mfig(5, 3.5)
        for i, (idx, val) in enumerate(cat.items()):
            ax3.barh(idx, val / 1e3, color=[C2, C3, C4][i % 3], height=0.5, zorder=3, edgecolor='none')
            ax3.text(val / 1e3 + 1.5, i, f'${val/1e3:.0f}K', va='center',
                     color=C5, fontsize=10, fontweight='800')
    ...
    with col3:
        ...
        reg_sel = st.multiselect("Select regions", df['Region'].unique().tolist(),
                                  ...
                                  key="reg_select", label_visibility='collapsed')
        reg = df[df['Region'].isin(reg_sel)].groupby('Region')['Sales'].sum().sort_values()
        fig4, ax4 = mfig(5, 3.5)
        for i, (idx, val) in enumerate(reg.items()):
            ax4.barh(idx, val / 1e3, color=[C1, C2, C3, C4][i % 4], height=0.5, zorder=3, edgecolor='none')
            ax4.text(val / 1e3 + 1.5, i, f'${val/1e3:.0f}K', va='center',
                     color=C5, fontsize=10, fontweight='800')
...
# ══════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<p>Prophet model projections with confidence intervals</p>", unsafe_allow_html=True)

    col1, col2, col3 = st.columns([2, 2, 1.5])
    with col1:
        seg_type = st.selectbox("Segment Type", ["Category", "Region"], key="seg_type", label_visibility='collapsed')
    with col2:
        opts = df['Category'].unique().tolist() if seg_type == "Category" else df['Region'].unique().tolist()
        seg_val = st.selectbox(f"Select {seg_type}", opts, key="seg_val", label_visibility='collapsed')
        seg_df = df[df['Category'] == seg_val] if seg_type == "Category" else df[df['Region'] == seg_val]
    with col3:
        horizon = st.slider("Forecast Horizon", 1, 6, 3, key="horizon")
    ...
    st.markdown("""<div class='section-hdr'>
        <span class='sec-icon'>📊</span>
        <span class='sec-title'>Revenue Forecast — {seg_type}</span>
        <span class='sec-badge' style='margin-left: auto;'>95% Confidence</span>
    </div>""".format(seg_type=seg_val))

    fig, ax = mfig(13, 5)
    ax.plot(seg_m['ds'], seg_m['y'] / 1e3, color=C2, lw=2.8, label='Actual Revenue', zorder=3)
    ax.fill_between(seg_m['ds'], seg_m['y'] / 1e3, alpha=0.12, color=C2)

    fo = fc.tail(horizon)
    ax.plot(fo['ds'], fo['yhat'] / 1e3, color=C4, lw=2.8, marker='o',
            markersize=10, label='Forecast', zorder=5, markeredgecolor=BG, markeredgewidth=2.5)
    ax.fill_between(fo['ds'], fo['yhat_lower'] / 1e3, fo['yhat_upper'] / 1e3,
                    alpha=0.25, color=C4, label='95% Confidence Band', zorder=2)
    ax.axvline(seg_m['ds'].iloc[-1], color=C3, ls=':', lw=1.8, alpha=0.6)

    for _, row in fo.iterrows():
        ax.annotate(f"${row['yhat']/1e3:.0f}K",
                    (row['ds'], row['yhat'] / 1e3),
                    xytext=(0, 14), textcoords='offset points',
                    ha='center', color=C4, fontsize=9, fontweight='800')
    ...
    st.markdown("""<div class='section-hdr'>
        <span class='sec-icon'>📐</span>
        <span class='sec-title'>Model Accuracy</span>
    </div>""")
    ...
    metrics = [
        ...
        (m3, "Month +1", f"${fo['yhat'].iloc[0]/1e3:.0f}K", "Next Month", "📅"),
        (m4, "Month +{horizon}", f"${fo['yhat'].iloc[-1]/1e3:.0f}K", f"+{horizon}M Outlook", "🔮"),
    ]
    ...
    for col, lbl, val, sub, icon in metrics:
        with col:
            st.markdown(f"""<div class='stat-card'>
                ...
                <div class='stat-label'>{lbl}</div>
                <div class='stat-value' style='font-size: 28px;'>{val}</div>
                <div class='stat-sub'>{sub}</div>
            </div>""", unsafe_allow_html=True)

    st.markdown("""<div class='section-hdr'>
        <span class='sec-icon'>📋</span>
        <span class='sec-title'>Forecast Breakdown</span>
    </div>""")
...
# ══════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<p>Isolation Forest + Z-Score detection on weekly sales data</p>", unsafe_allow_html=True)
    ...
    anomaly_stats = [
        (c1, "Isolation Forest", str(len(iso_an)), "Flags", "dn", "🌲"),
        (c2, "Z-Score Anomalies", str(len(z_an)), "Flags", "dn", "📊"),
        (c3, "Consensus Flags", str(both), "Agree", "up", "✅"),
        (c4, "Weeks Analyzed", str(len(ws)), "Data", "", "📅"),
    ]
    ...
    st.markdown("""<div class='section-hdr'>
        <span class='sec-icon'>📉</span>
        <span class='sec-title'>Weekly Sales — Anomaly Detection</span>
        <span class='sec-badge'>Isolation Forest model · 4-week rolling average overlay</span>
    </div>""", unsafe_allow_html=True)
    ...
    fig, ax = mfig(13, 5)
    ax.fill_between(ws.index, ws.values / 1e3, alpha=0.1, color=C2)
    ax.plot(ws.index, ws.values / 1e3, color=C2, lw=1.8, alpha=0.95, label='Weekly Revenue', zorder=2)

    rm_plot = ws.rolling(4).mean() / 1e3
    ax.plot(ws.index, rm_plot, color=C3, lw=1.8, ls='--', alpha=0.7, label='4-wk Rolling Avg', zorder=2)

    spikes = anom[anom > ws.mean()]
    drops = anom[anom <= ws.mean()]
    if len(spikes):
        ax.scatter(spikes.index, spikes.values / 1e3, color=UP, s=140, zorder=5,
                   marker='^', label='Spike anomaly', edgecolors=BG, lw=2)
    if len(drops):
        ax.scatter(drops.index, drops.values / 1e3, color=DN, s=140, zorder=5,
                   marker='v', label='Drop anomaly', edgecolors=BG, lw=2)
    ...
    st.markdown("""<div class='section-hdr'>
        <span class='sec-icon'>📋</span>
        <span class='sec-title'>Anomaly Log</span>
    </div>""")
...
# ══════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<p>K-Means clustering · 4 behavioral segments identified</p>", unsafe_allow_html=True)
    ...
    st.markdown("""<div class='section-hdr'>
        <span class='sec-icon'>🎯</span>
        <span class='sec-title'>PCA Cluster Projection</span>
        <span class='sec-badge'>K-Means k=4 · X-axis: sales volume · Y-axis: demand stability</span>
    </div>""", unsafe_allow_html=True)

    fig, ax = mfig(13, 6)
    ax.grid(axis='both', color=GRD, linewidth=0.5, linestyle='--', alpha=0.3)
    for seg, grp in feat.groupby('Segment'):
        ax.scatter(grp['x'], grp['y'], label=seg, color=pal[seg],
                   s=180, edgecolors=PNL, lw=2, alpha=0.9, zorder=4)
        for _, row in grp.iterrows():
            ax.annotate(row['Sub-Category'], (row['x'], row['y']),
                        fontsize=8.5, color=C5, ha='center', va='bottom',
                        xytext=(0, 8), textcoords='offset points', fontweight='700')
    ax.set_xlabel(f"PC1 — {pca.explained_variance_ratio_[0]*100:.0f}% variance", fontsize=11, fontweight='700', color=TK)
    ax.set_ylabel(f"PC2 — {pca.explained_variance_ratio_[1]*100:.0f}% variance", fontsize=11, fontweight='700', color=TK)
    legend = ax.legend(facecolor=BG, edgecolor=GRD, labelcolor=C5, fontsize=10, loc='lower right', framealpha=0.95)
    finish(fig)
    st.pyplot(fig)

    st.markdown("""<div class='section-hdr'>
        <span class='sec-icon'>📋</span>
        <span class='sec-title'>Segment Strategies</span>
    </div>""")

    strat = {
        '🏆 High Volume Stable': 'Automate replenishment · maintain safety stock',
        '📈 Growing Demand': 'Increase forecast buffer 15–20% · expand SKUs',
        '⚡ High Volatility': 'Safety stock ×2 · weekly demand sensing',
        '📉 Declining Demand': 'Reduce orders · markdown aging inventory',
    }
    bord = [C2, UP, C4, DN]
...
st.markdown(f"""
<div style='text-align: center; color: #7691c0; font-size: 11px; padding: 20px 0; font-weight: 600;'>
    <p>Made with ❤️ using Streamlit · Data: {df['Order Date'].dt.year.min()}–{df['Order Date'].dt.year.max()} · Last updated: just now</p>
</div>
""", unsafe_allow_html=True)
